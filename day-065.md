...
Change `/root/code/serving/canary_deploy.py` to:
...
Go to `cd /root/code/serving/` and run `python3 canary_deploy.py`

```bash
root@controlplane ~/code/serving via 🐍 v3.12.3 ➜  python3 canary_deploy.py
Phase 1: v1=95% v2=5%
...
```bash
root@controlplane ~/code/serving via 🐍 v3.12.3 ➜  python3 canary_deploy.py | grep "Phase 2" -A 1
Phase 2: v1=70% v2=30%
  v1_requests=70 v2_requests=30 v2_error_rate=0.00%
```

Test rollback scenario (simulate broken v2):
...
ROLLBACK_THRESHOLD = 0.05
REQUESTS_PER_PHASE = 100
V2_ERROR_RATE = 0.10  # 10% error - will trigger rollback
...
class CanaryDeployer:
    ...
    def promote(self) -> tuple[float, float]:
        ...
        if self.phase == 1:
            self.v1_weight = 0.95
            self.v2_weight = 0.05
EOF print(f"Total requests: {total_requests}")s}")num} phase(s)").2%} "
Phase 1: v1=95% v2=5%
  v1_requests=94 v2_requests=6 v2_error_rate=0.00%
Phase 2: v1=70% v2=30%
  v1_requests=70 v2_requests=30 v2_error_rate=10.00%
  ROLLBACK — v2 error rate 10.00% > threshold 5%
OUTCOME: ROLLED_BACK after 2 phase(s)
Total requests: 200
```
